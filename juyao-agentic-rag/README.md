...
| `juyao-rag` | 命令行单次问答 |
| `juyao-rag-eval` | RAGAS 离线测评（见 [docs/eval/](docs/eval/GETTING_STARTED.md)） |
| `juyao-rag-api` | 启动 FastAPI 服务（默认 `0.0.0.0:8000`） |
| `juyao-rag-kafka-consumer` | Kafka 消费异步入库 |

等价模块调用：`python -m rag_core.cli.ingest`、`python -m rag_eval.cli.main` 等。

## RAGAS 测评

测评代码位于 `src/rag_eval/`（与 `rag_core` 并列），人工标注数据集在 `src/rag_eval/datasets/`。
...
详见 [测评快速启动](docs/eval/GETTING_STARTED.md) · [测评流程](docs/eval/WORKFLOW.md) · [指标说明](docs/eval/METRICS.md)。
...
| [eval/GETTING_STARTED.md](docs/eval/GETTING_STARTED.md) | RAGAS 测评安装与启动 |
| [eval/WORKFLOW.md](docs/eval/WORKFLOW.md) | 测评完整流程 |
| [eval/METRICS.md](docs/eval/METRICS.md) | RAGAS 指标说明 |
