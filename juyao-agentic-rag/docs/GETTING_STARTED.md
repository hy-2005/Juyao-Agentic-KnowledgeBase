...
详见 [AGENT_FLOW.md](./AGENT_FLOW.md)（图谱搜索链路）与 [docs/README.md](./README.md)（设计文档索引）。
...
FastAPI 接口分四组（对话 + 会话 + 管理端 chunks/graph + 内部入库），Swagger：`http://127.0.0.1:8000/docs`。文档上传不在 FastAPI，而走 Java `/rag/documents/*` → Kafka。详见 [API.md](./API.md)。
