...
> 状态：🔄 进行中（父子分块/结构化识别已完成并启用；PDF 表格布局/子块大小验证遗留） · 创建：2026-08-07 · 更新：2026-08-12
...
**已启用**（config/local.toml，本机生效）：
- `chunk_parent_enabled = true`
- `child_chunk_size = 300`（原 200 偏小：中文公告/合同子块语义不完整）

**触发原因**：切片管理页发现入库文档无父子展示——全库 381 个 chunk 全部 `chunk_type=None`（开关从未开启）；且存在 4311 字符大块（超 `chunk_max_chars=1400` 三倍）+"交易即开票"被从中间切断（"易即开票"）的切分质量问题。

**注意事项**：
- 开启只对**之后入库**的文档生效；存量 381 个普通 chunk 需重新上传/重灌才变父子
- 4311 字符大块的根因待查（`enforce_max_span_length` 理论上应按 1400 硬切，疑似入库时配置非当前值或切分路径差异）——遗留排查项
