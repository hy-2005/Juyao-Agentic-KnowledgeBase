...
> 状态：🔄 进行中（预分批/规则主通道/父子分块/扫描件 OCR 已完成；图像/表格结构/层级解析待办，见 §5.5 调研） · 创建：2026-08-07 · 更新：2026-08-07
...
## 5. 多模态扩展路径（现状：OCR 已实现，图像/表格结构未实现）

> ✅ 现状澄清（2026-08-07 更新）：扫描件 OCR **已实现并实测生效**——`infrastructure/loaders.py:59-134` 有完整链路（页面文本 <20 字符且含图片判定为扫描件页 → RapidOCR 懒加载 → 200DPI 渲染 PNG → 按行拼接）。`rapidocr-onnxruntime` 已装入 venv（2026-08-07），实测 `扫描件合同.pdf`：0 字符 → **61 字符**（合同编号/甲乙双方/标的均提取成功，耗时 ~6s）。
> 仍缺失：图像（非扫描件图片）抽取、表格布局还原、PDF 标题层级、图像 embedding。生成模型 qwen3.6-35b-a3b 为纯文本模型。
...
| 问题 | 现象 | 根因 | 状态 |
|---|---|---|---|
| 扫描件 PDF 内容静默丢失 | `扫描件合同.pdf` 实测提取 0 字符 | `rapidocr-onnxruntime` 未装，`_get_ocr_engine` 返回 None 只打告警不报错 | ✅ 已解决（2026-08-07 安装依赖，实测 61 字符） |
| PDF 表格布局还原缺失 | 单元格边界丢失，行列关系断裂 | loader 仅 `get_text()` 抽文本流 | ❌ 待解决（见 §5.5 调研） |
| PDF/Word 标题层级缺失 | 标题塌缩为扁平文本 | 加载后样式信息（字号/加粗）丢失 | ❌ 待解决（见 §5.5 调研） |
| JSON/XML/HTML 无结构解析 | 仅按纯文本读入 | loader 无对应解析分支 | ❌ 待解决（低优先级） |

### 5.5 企业主流方案调研（2026-08-07，不手搓的替代路线）

调研结论：企业 RAG 的 PDF 解析已从"文本提取"演进为"结构+语义+元数据"理解，主流路线分三档：

| 路线 | 代表方案 | 特点 | 适配度 |
|---|---|---|---|
| 轻量开源解析器 | [DocSlicer](https://github.com/DocSlicer/DocSlicer)（确定性解析+标题层级树 `hierarchy.to_outline()`+表格 JSONL）、[ZhDocParser](https://github.com/melonelish/ZhDocParser)（中文优先、标题路径、表格上下文、页眉页脚过滤，输出 MD/JSON/chunks.json） | 自托管、确定性、可插拔替换现有 `loaders.py` | 高（改动小，替换 loader 即可） |
| AI 版面分析全家桶 | PP-StructureV2 / PaddleOCR（版面分析+表格识别+OCR 一体化）、LayoutLMv3 | 扫描件+复杂版面效果好，模型重 | 中（需引入 Paddle 依赖，较重） |
| 商业解析 API | [TextIn（合合信息）](https://www.textin.com/news/20260304114004396) 等 | 复杂表格还原 92%+、稳定率 99.99%、可本地部署 | 高（效果好，需付费/授权） |

工程实践共识：**动态策略路由**——扫描件走"OCR+结构重建"，可编辑 PDF 走"原生解析+样式修复"，按文档类型分流；解析结果统一输出 Markdown/JSON（表格还原、标题层级树），再接入现有切分链路。

**推荐路径**：先评估 DocSlicer / ZhDocParser 替换 `loaders.py`（解决表格+标题层级，零模型成本）；不满足再上商业 API。检索侧配套：分层嵌入（表头+行列组合结构化文本再嵌入）可复用现有 Qdrant/ES。
