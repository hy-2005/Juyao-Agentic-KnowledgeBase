【合同】本任务遵循 `kg_entity_relation_contract` 共享合同（实体规范化、谓词闭集、JSON schema、抽取哲学等）。下方仅描述问句侧特有指令。
...
- `relation_predicate`：若能从问题推断关系，填最短中文谓词（位于、隶属于、归口管理、签署、适用于…）；**完全不确定时允许 `""`**（实体种子）。
