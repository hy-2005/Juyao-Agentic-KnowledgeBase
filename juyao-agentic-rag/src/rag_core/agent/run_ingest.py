# 知识入库入口：文本文件 → 切块 → 写入 Qdrant 与 Elasticsearch（混合检索）。
#
# 本地开发：在项目根执行 pip install -e . 后：
# python -m rag_core.agent.run_ingest --file src/data/samples/sample_medical.txt（或使用 juyao-ingest）。

import argparse
import uuid
from pathlib import Path
from tqdm import tqdm
from rag_core.rag.elasticsearch_store import sync_chunks_to_elasticsearch
...
def ingest_file(file_path: str) -> int:
    ...
    # 全文侧：同一批 chunks 写入 ES（_id=chunk_id），与向量侧并行构成混合检索数据面；地址/索引名见 config。
